...
def get_release_assets(tag_name: str = ODIFF_VERSION) -> list[dict[str, Any]]:
    ...
    """
    resp = httpx.get("https://api.github.com/repos/dmtrKovalenko/odiff/releases")
    ...
    for asset in assets:
        if (
            ...
            or (
                system == "darwin"
                and (
                    (processor == "arm64" and asset["name"] == "odiff-macos-arm64.exe")
                    or (processor == "i386" and asset["name"] == "odiff-macos-x64.exe")
                )
            )
        ):
            return asset["browser_download_url"]
    msg = "Couldn't find odiff binary for your system."
    raise ValueError(msg)
